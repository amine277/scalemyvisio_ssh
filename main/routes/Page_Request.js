...
router.post("/RoomViewersList", async function (req, res) {
  var Id = req.body.Id;
  var roomId = req.body.roomId;

  const room = await Room.findOne({ name: roomId });
  if (room) {
    var list = room.viewers;
    console.log(list);
...
router.post("/exitRoomV", async function (req, res) {
  var user_name = req.body.name;
  var roomName = req.body.roomId;
  var Id = req.body.Id;
  ...
  if (room) {
    room.viewers.pop({ Id: Id, username: !null });
    const savedRoom = await room.save();
  }
  res.send({ value: true });
});
...
router.get("/ChooseStream", (req, res) => {
  try {
    res.sendFile("ChooseStream.html", {
